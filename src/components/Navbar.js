import React from "react";
import { Link } from "react-router-dom";

const Navbar = () => {
  const mystyle = {
    color: "white",
    backgroundColor: "black",
  };
  return (
    <div>
      <nav className="navbar fixed-top navbar-expand-lg " style={mystyle}>
        <div className="container-fluid">
          <Link className="navbar-brand" to="/" style={mystyle}>
            My Movies
          </Link>
          <button
            className="navbar-toggler"
            type="button"
            data-bs-toggle="collapse"
            data-bs-target="#navbarSupportedContent"
            aria-controls="navbarSupportedContent"
            aria-expanded="false"
            aria-label="Toggle navigation"
          >
            <span className="navbar-toggler-icon"></span>
          </button>
          <div
            className="collapse navbar-collapse text-primary"
            id="navbarSupportedContent"
          >
            <ul className="navbar-nav me-auto mb-2 mb-lg-0 text-primary">
              <li className="nav-item text-white">
                <Link
                  className="nav-link active"
                  aria-current="page"
                  to="/"
                  style={mystyle}
                >
                  Home
                </Link>
              </li>
              <li className="nav-item" style={mystyle}>
                <Link className="nav-link" to="/top_rated" style={mystyle}>
                  Top-rated
                </Link>
              </li>
              <li className="nav-item">
                <Link className="nav-link" to="/upcoming" style={mystyle}>
                  Upcoming
                </Link>
              </li>
              <li className="nav-item">
                <Link className="nav-link" to="/popular" style={mystyle}>
                  Popular
                </Link>
              </li>
            </ul>
            <form className="d-flex" role="search">
              <input
                className="form-control me-2"
                type="search"
                placeholder="Search"
                aria-label="Search"
              />
              <button className="btn btn-outline-success" type="submit">
                Search
              </button>
            </form>
          </div>
        </div>
      </nav>
    </div>
  );
};

export default Navbar;
